 ...
 * 128Hz 단독 측정 전용 코드.
 *
 * 다른 센서 검증과 동시에 실행하지 않고 이 Core 하나만 사용해야 한다.
 * 다른 주파수의 원본을 재사용하지 않고 samplingPeriodUs=7813으로 센서를 별도 등록한다.
 * Android가 실제로 전달한 원본 timestamp 간격과 실측 Hz를 센서별로 계산한다.
 */
class Requested128HzCore(
    context: Context,
    private val onOriginal: (OriginalSample) -> Unit,
) : SensorEventListener {
    companion object {
        const val REQUESTED_HZ = 128
        const val SAMPLING_PERIOD_US = 7813
        const val OUTPUT_KEY = "standalone128"
        const val OUTPUT_FILE_NAME = "128Hz_단독측정.txt"
