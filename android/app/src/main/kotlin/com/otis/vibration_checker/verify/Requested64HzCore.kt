 ...
 * 다른 주파수의 원본을 재사용하지 않고 samplingPeriodUs=15625로 센서를 별도 등록한다.
 * Android가 실제로 전달한 원본 timestamp 간격과 실측 Hz를 센서별로 계산한다.
 */
class Requested64HzCore(
    context: Context,
    private val onOriginal: (OriginalSample) -> Unit,
) : SensorEventListener {
    companion object {
        const val REQUESTED_HZ = 64
        const val SAMPLING_PERIOD_US = 15625
        const val OUTPUT_KEY = "standalone64"
        const val OUTPUT_FILE_NAME = "64Hz_단독측정.txt"
