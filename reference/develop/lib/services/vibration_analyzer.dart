...
class VibrationAnalyzer {
  ...
  VibrationResult analyze(List<SensorSample> samples) {
    if (samples.length < 2) {
      throw ArgumentError('怨꾩궛?섎젮硫?理쒖냼 2媛??댁긽???쇱꽌 ?섑뵆???꾩슂?⑸땲??');
    }
  ...
  AxisRideResult _axisRideResult(List<double> values) {
    if (values.length < 2) {
      throw ArgumentError('異뺣퀎 由ы룷??怨꾩궛???꾪븳 媛믪씠 遺議깊빀?덈떎.');
    }
  ...
  double _peakToPeak(List<double> values) {
    if (values.isEmpty) {
      throw ArgumentError('Peak-to-Peak 怨꾩궛???꾪븳 媛믪씠 ?놁뒿?덈떎.');
    }
  ...
  double _average(List<double> values) {
    if (values.isEmpty) {
      throw ArgumentError('?됯퇏 怨꾩궛???꾪븳 媛믪씠 ?놁뒿?덈떎.');
    }
  ...
  double _percentile(List<double> sortedValues, double percentile) {
    if (sortedValues.isEmpty) {
      throw ArgumentError('A95 怨꾩궛???꾪븳 媛믪씠 ?놁뒿?덈떎.');
    }
