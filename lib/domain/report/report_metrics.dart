...
///           정해져 그 값이 채워지면 이 파일을 고치지 않고 나온다
///
///       임시값을 만들어 채우지 않는다. 재지 못한 자리는 표에 `—` 로
///       나오고 신호등은 회색이 된다.
///
///       소음 두 값을 서로 다른 데서 가져오는 까닭
///         소음 0 은 값이 아니라 미측정 표시라, 두 값 모두 0 인 표본을
///         빼고 센다. 다만 빼는 자리가 다르다.
///         - 최대는 `MeasurementResult.noiseMax` 를 그대로 읽는다. 그 값이
///           소음 최대 지표의 정본이고, 여기서 다시 계산하지 않는다.
///           다시 세면 0 을 빼는 규칙이 어셈블러와 갈라져 두 값이 어긋난다
///         - 평균은 시계열에서 낸다. 모든 표본이 있어야 나오는 값이라
///           어셈블러가 미리 줄여 둘 수 없다
class ReportMetrics {
  /// 평균 소음 (dBA)
  ...
  /// 근거: 측정 — `test/fixtures/noise_warmup.txt` (44100Hz · 1초 창으로
  ///       고정한 뒤 뽑은 14,530행 측정)에서 소음 열 앞 43개 표본이
  ///       0 이다. 256Hz 로 0.168초 어치이고, 0 은 그 앞쪽에만 있다.
  ///       개수를 코드에 박지 않으므로 기기나 설정이 달라져 개수가 변해도
  ///       이 함수는 그대로 돈다.
  ///       미확인: 0 이 끝난 뒤로도 값이 한동안 낮게 올라온다. 같은
  ///       파일에서 0 이 아니면서 35dBA 에 못 미치는 표본이 303개(약
  ///       1.2초)이고, 그 구간을 평균에 넣으면 평균이 0.61dBA 낮아진다.
  ///       0 만 걸러서는 이 구간이 남는데, 어디까지를 덜 찬 값으로 볼지는
  ///       정하지 못했다
