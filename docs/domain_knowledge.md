...
- **변경 이력 (Change Log)**:
  ...
  - 2026-07-04: 텀블링→슬라이딩 전환 및 윈도우 0.5→1.0s. 0.5s 텀블링은 구간 경계 0.1s 이동에 Z Aptp가 22.6→16.8로 요동(위상 민감)하여 신뢰 불가. 슬라이딩 1.0s는 위상 무관하게 목표값에 수렴.
  - 2026-07-04: 현재 Aptp 축 순서 X<Z<Y (21.8/25.7/32.6). 문서 정답은 X<Y<Z (8.2/12.9/22.2)로 Z가 최대여야 함. 수평축(X/Y) 과대산출로 Y가 최대가 되는 역전 발생. 원인은 EVA 축별 주파수 가중 필터 부재. Phase 7 OI-1 에서 축별 가중 피팅 시 해소 예정. Phase 1 범위 아님.
  - 2026-07-04: 마이크 오프셋 파라미터화(`micDbfsToDbaOffset` = 85.0). dBA = dBFS + 85.0 + calibrationOffsetDba 식의 85.0은 실측 캘리브레이션 전 임시 추정치(모바일 PCM dBFS->SPL 근사값)로 기기별 보정 없이는 절대값 신뢰 불가(Phase 7 OI-4 과제). 골든 raw(2024F1447R01.txt)는 이미 캘리브레이션된 데이터이므로 본 환산식과 무관함.
  - 2026-07-04: TYPE_LINEAR_ACCELERATION 사용 당위성 검증. 골든 샘플(2024F1447R01.txt)의 Z축 평균이 -0.001 mg로 중력(9806.65 mg)이 완벽히 제거된 상태임을 검증함. 따라서 안드로이드 네이티브 단에서 TYPE_LINEAR_ACCELERATION(중력 제거 선형가속도)을 구독하는 것이 물리적으로 정당함. 또한 라이브 속도 표시용 첫 1초 보정과 엔진 analyze()의 최종 분석용 보정은 서로 완전히 독립된 버퍼/스코프에서 동작하므로 보정이 이중으로 중복 적용되지 않음을 확인.
  - 2026-07-04: Phase 3 시점 한계: 결과 화면(result_screen)은 방금 측정한 건(lastResultId)만 조회 가능. 과거 결과 조회는 Phase 4 Repository 의존.
  - 2026-07-04: Phase 4 완료 및 유지되는 오픈 이슈: 1) 실기기 센서/dBA 실측 (Phase 2 PENDING) — 실기기 확보 시 최우선 검증. 2) 표 및 리포트의 X/Y Aptp 과대 산출(21.81/32.57 mg)은 OI-1 과제로 Phase 7 캘리브레이션 대기 중이며, 현재 리포트는 이 미보정 실측치를 정직하게 표시하고 있음을 명기.
  - 2026-07-04: Phase 5 진행 중 오픈 이슈 명기: 1) 관리자 ID 활성/비활성(OI-2)은 서버 확정 대기이므로 `AuthRepository` 인터페이스에 자리만 두고 Local 구현은 항상 enabled 로 처리. 2) 안드로이드 이메일 첨부 FileProvider(`file_paths.xml` 및 `<queries>`)를 선제 설정했으나, 실기기에서 메일 앱 작성창에 첨부파일이 실제로 정상 로드되는지는 기기 확보 시 Phase 2 실측과 함께 검증할 오픈 이슈로 등록.
...
- **기종별 실측 메모 (테이블)**:
  ...
  |---|---|---|---|---|---|
  | 에뮬레이터 / 모의 | Android | - (Mock) | 256 Hz (고정) | 마이크 오프셋 미검증 (임시값 85.0 사용) | Mock 스트림 / 실기기 미연결 |
  | (대기중: OI-3 법인폰/협력업체폰) | - | 실측 미검증 | 진단 로깅 코드 준비됨, 실측 대기 | 마이크 오프셋 미검증 | Phase 3 병행 가능, 실기기 확보 즉시 최우선 검증(오픈 이슈) |
